...
public class Program {
	...
	public void start(String[] urls, ArrayList<String> log, String pathDestination, String pathDatabase, String APIKey)
		...
		try {
			...
			log.add("Updating database.");

			log.add("Get database.");
			XSSFWorkbook database = ex.openExcelFile(pathDatabase); // get 'database.xlsx'

			log.add("Get template.");
			XSSFWorkbook template = ex.openExcelFile(pathDestination); // get 'template.xlsx'
			...
			BufferedOutputStream bufferedOutTemplate = new BufferedOutputStream(
					new FileOutputStream(new File(pathDestination)), 8 * 1024); // buffered output stream, buffer size
																				// 8*1024
																				// bytes

			log.add("Update all webs in database.");
			ex.updateDatabase(database, sheetDatabase, sheetCurrentTechSummary, bufferedOutTemplate);// update "ALL
																										// WEBs" in
																										// database.xlsx

			log.add("Database updated successfully.");

			ex.copyDatabase(database, template, sheetDatabase, bufferedOutTemplate);

			log.add("Database copied successfully.");
			...
			log.add("Copied all webs data from database to template successfully.");

			ex.copyTemplate(database, template, sheetAllTech, sheetCurrentTechSummary, bufferedOutTemplate);// update
																											// "All
																											// Tech
																											// (Current
																											// &
			...
			log.add("Copied all tech from database to template successfully.");
			...
			log.add("Database update process successful.");
			...
			// XSSFWorkbook template = ex.openExcelFile(pathDestination); // open streams
			...
			// FileOutputStream(pathDestination);//output stream

			log.add("Inserting domains to summary.");

			ex.insertDomain(urls, pathDestination, template, sheetCurrentTechSummary, sheetNewTech, sheetTechBWName,
					sheetDatabase);

			log.add("Inserting domains to all tech.");

			ex.insertDomain(urls, pathDestination, template, sheetAllTech);
			bufferedOutTemplate.flush();
			//
			...
			for (int i = 0; i < totalNumDomain; i++) {
				...
				ex.writeTechName(names, rowNum, colNum, template, sheetTechBWName); // write the tech to spreadsheet
																					// "Tech
																					// BW Name"
			...
			log.add("Audit generation completed successfully.");
			System.out.println("success");

			long end = System.currentTimeMillis();

			log.add("Time taken: " + (end - start) / 1000 + " s");
			System.out.println("time taken: " + (end - start) / 1000 + " s");

		} catch (Exception oex) {
			System.out.println(String.format("Exception occurred: %1$s", oex.getMessage()));
			log.add(String.format("Exception occurred: %1$s", oex.getMessage()));
		}
