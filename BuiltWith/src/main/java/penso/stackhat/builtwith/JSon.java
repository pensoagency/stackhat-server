...
//			System.out.println(wrongURL);
	...
	/**
	 * get all the tech used in the website with specific tags = { "analytics", "mobile", "cms", "cdn", "hosting", "mx", "Web Server", "feeds",
			"ssl", "ads", "media", "payment", "mapping" }
	 * @author Yuxuan
	 * @param JSon   generated by Builtwith API, in String format
	 * @return ArrayList contains the names of all techniques used for each domain
	 */
	public ArrayList<String> getInterestedTechNames(String JSon) {

		this.interestedNames = new ArrayList<String>();
		// create Json object using json received from "getJson" method
		JSONObject dataJson = new JSONObject(JSon);
...
//		boolean flag = true;
		...
		for (String tag : interestedTags) {
			for (int i = 0; i < Technologies.length(); i++) {
				...
				if (tag.equals(Tag)) {
//					if(flag==true) {
//						interestedNames.add("******"+Tag+"******");
//						flag=false;
//					}
					String Name = obj.getString("Name");
					interestedNames.add(Name);
				}
			}
//			flag=true;
		}
